  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (!(uploadResponse.data as any).success) {
        throw new Error(
          (uploadResponse.data as any).message || "Upload failed",
        );
      }
      ...
      if (!(analyzeResponse.data as any).success) {
        throw new Error(
          (analyzeResponse.data as any).message || "Analysis failed",
        );
      }
    ...
    <div className="max-w-xl mx-auto">
      <div className="border-2 border-dashed border-red-900/30 rounded-lg p-10 text-center hover:border-red-500/70 transition-all">
        ...
        <div className="mt-6 max-w-md mx-auto">
          ...
          >
            <option value="entry">
              Entry Level (Student / Fresher / 0-2 years)
            </option>
  ...
  return (
    <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleFilterChange("days_posted", parseInt(e.target.value))
                }
                min="1"
                max="90"
                className="w-full px-4 py-2 bg-dark-800 rounded-lg border border-red-900/30 focus:border-red-500 outline-none"
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          <p className="text-gray-400">
            No job recommendations found. Try adjusting your filters.
          </p>
        </div>
      ) : (
                    ...
                    {(job as any).matchLevel && (
                      <div
                        className={`mt-2 px-3 py-1 rounded-full text-xs font-medium ${
                          (job as any).matchLevel === "excellent"
                            ? "bg-green-500/20 text-green-400 border border-green-500/30"
                            : (job as any).matchLevel === "very-good"
                              ? "bg-blue-500/20 text-blue-400 border border-blue-500/30"
                              : (job as any).matchLevel === "good"
                                ? "bg-yellow-500/20 text-yellow-400 border border-yellow-500/30"
                                : (job as any).matchLevel === "fair"
                ...
                <div className="mb-4 flex gap-4 text-xs">
                  ...
                  {(job as any).jobLevel && (
                    <div className="flex-1 bg-black/30 rounded p-2 border border-red-900/20">
                      ...
                      <div className="text-red-400 font-bold capitalize">
                        {(job as any).jobLevel}
                      </div>
                    </div>
                  )}
          ...
          <section className="py-8">
            <div className="max-w-4xl mx-auto text-center mb-12">
              <h1 className="text-4xl md:text-5xl font-bold mb-6">
                <span className="text-red-500">Optimize</span> Your Resume for
                ATS
              </h1>
              <p className="text-gray-300 text-lg">
                Upload your resume to get instant feedback and personalized job
                recommendations
              </p>
            </div>
