...
/**
 * POST /api/jobs/archive-stale
 * Soft-archive stale jobs that have not been seen recently
 */
router.post('/jobs/archive-stale', authenticateToken, async (req, res) => {
  try {
    ...
    const archivedCount = await joobleService.archiveStaleJobs(days);
